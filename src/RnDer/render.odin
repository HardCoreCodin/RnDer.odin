package RnDer


// Frame Buffer:
// =============

Color :: struct #packed { B, G, R, A: u8 };
Pixel :: struct #raw_union { color: Color, value: u32};

PIXEL_SIZE :: 4;
RENDER_SIZE :: PIXEL_SIZE * 4 * 1024 * 1024;
RENDER_MAX_WIDTH :: 3840;
RENDER_MAX_HEIGHT :: 2160;

FrameBuffer :: struct {
    width, height: u16,
    size: u32,
    pixels: ^[RENDER_SIZE]Pixel
};

createFrameBuffer :: proc() -> ^FrameBuffer {
    frame_buffer: = Alloc(FrameBuffer);
    using frame_buffer;

    width = RENDER_MAX_WIDTH;
    height = RENDER_MAX_HEIGHT;
    size = u32(width * height);
    pixels = Alloc([RENDER_SIZE]Pixel);

    return frame_buffer;
}

RendererCallbacks :: struct {
    zoom,
    move,
    rotate,
    resize,
    render: EngineCallback
};

RendererType :: enum {
    RAY_TRACER,
    RAY_CASTER
}

Renderer :: struct {
    type: RendererType,
    title: cstring,
    on: RendererCallbacks
};

RayHit :: struct { 
    distance: f32,
    position, normal: ^vec3
};

// Text:
// =====

LINE_HEIGHT :: 12;
FIRST_CHAR_CODE :: 32;
LAST_CHAR_CODE :: 127;

drawText :: proc(using frame_buffer: ^FrameBuffer, str: []u8, color: u32, x, y: u16) {
    if x < 0 || x > (width - FONT_WIDTH) ||
       y < 0 || y > (height - FONT_HEIGHT) do
        return;

    current_x := u16(x);
    current_y := u16(y);
    offset: u16 = 0;
    
    pixel_line_step: u32 = u32(width - FONT_WIDTH);
    char_line_step : u32 = u32(width * LINE_HEIGHT);

    pixel_index: u32 = u32(width * y + x);
    chr_pixel_index: u32;
    byte_index: u32;
    current_byte: u8;

    for chr in str {
        switch chr {
            case '\n':
                if current_y + FONT_HEIGHT > height do
                    return;

                pixel_index += char_line_step - u32(current_x + x);
                current_x = x;
                current_y += LINE_HEIGHT;

            case '\t':
                offset = FONT_WIDTH * (4 - ((current_x / FONT_WIDTH) & 3));
                current_x += offset;
                pixel_index += u32(offset);

            case:
                if chr < FIRST_CHAR_CODE ||
                   chr > LAST_CHAR_CODE do 
                    continue;
                   
                byte_index = u32(FONT_WIDTH * (chr - FIRST_CHAR_CODE));
                chr_pixel_index = pixel_index;
                for h: u8 = 0; h < FONT_HEIGHT; h += 1 {
                    current_byte = Font[byte_index];
                    for w: u8 = 0; w < FONT_WIDTH; w += 1 {
                        /* skip background bits */
                        if current_byte & (0x80 >> w) != 0 do
                            pixels[chr_pixel_index].value = color;

                        chr_pixel_index += 1;
                    }
                    byte_index += 1;
                    chr_pixel_index += pixel_line_step;
                }

                current_x += FONT_WIDTH;
                pixel_index += FONT_WIDTH;
        }
    }
}

printNumberIntoString :: proc "contextless" (
    number: u16, 
    slice: []u8
) {
    last := len(slice) - 1;
    if number == 0 {
        for i in 0..<last do slice[i] = ' ';
        slice[last] = '0';
    } else {
        prior := number;
        current := number;
        for i := last; i >= 0; i -= 1 {
            if current > 0 {
                prior = current;
                current /= 10;
                slice[i] = u8('0' + prior - current * 10);
            } else do slice[i] = ' ';
        }
    }
}

FONT_HEIGHT :: 8;
FONT_WIDTH :: 8;

Font: [768]u8 = {
        
};